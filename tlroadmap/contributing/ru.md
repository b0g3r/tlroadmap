...
Если по прямому клику по ссылке у вас всё открывается, а утилита утверждает, что сайт недоступен, то скорее всего сайт борется с краулерами и банит запросы от нашей утилиты или утилита не понимает его ответ. Обычно это ошибка 50х или `got a time out (response code 0)`. Варианта решения два:
  ...
  - Если этот сайт специфичен для этой компетенции, то замените markdown-ссылку на html-ссылку с аттрибутом `data-proofer-ignore`
